"""场景感知批采样器：同场景顺序样本成批、批间随机，减少视频随机 seek 与跨场景解码器切换。
...
说明: 每个场景先按数据集索引顺序切出完整批，再在批粒度打乱；各场景不足一批的尾样本集中补批，因而 drop_last
      只影响全数据集最后不足一批的样本。同场景顺序样本减少解码 seek，同时保持训练 step 的批间随机性。
"""
...
class SceneBatchSampler(Sampler[List[int]]):
    """把同场景顺序样本组成 batch，并按 batch 粒度打乱顺序。"""
