...
对外接口:
    ...
    - query_spawn_points(client, map_name, no_rendering_mode=False) -> list[list[6]]
    - load_scene_world(client, map_name, fixed_delta_seconds, tm_port, seed, no_rendering_mode=False) -> (world, tm)
...
说明: 仅支持 Opt 地图，并 unload ParkedVehicles 图层以规避静态车辆已知 API 问题（Design ⑪）。
      `no_rendering_mode` 在每次加载地图时写入 WorldSettings；严格同步模式 + 固定步长是
      多传感器对齐与可复现的前提（Design 同步要求）。
"""

import carla

from worker.geometry import transform_to_list
from worker.session.checks.session_checks import check_map_name, check_weather_preset
...
def _load_opt_map(client, map_name, no_rendering_mode=False):
    ...
    settings.no_rendering_mode = bool(no_rendering_mode)
    world.apply_settings(settings)
    return world


def query_spawn_points(client, map_name, no_rendering_mode=False):
    """加载地图并返回全部推荐生成点（路线队列的可达点来源，Design ③）。"""
    world = _load_opt_map(client, map_name, no_rendering_mode)
    return [transform_to_list(t) for t in world.get_map().get_spawn_points()]


def load_scene_world(client, map_name, fixed_delta_seconds, tm_port, seed, no_rendering_mode=False):
