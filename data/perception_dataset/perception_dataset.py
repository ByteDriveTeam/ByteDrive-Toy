...
对外接口:
    - PerceptionDataset(cfg) -> torch.utils.data.Dataset
        __getitem__(i) -> dict[str, Tensor]   # BGR uint8 / 紧凑标签 / depth_target / depth_inrange
        .frame_index -> list[(Path, int)]     # (场景目录, 帧号)，供可视化按场景筛选
说明: 单帧模型，每一帧独立成样本（采用所有帧，无开窗）。共享读取逻辑（索引/reader 缓存/RGB 紧凑布局）下沉到
...
class PerceptionDataset(SingleFrameSceneBase):
    ...
    每个样本为一帧：
        rgb           [3, H, W]  BGR uint8（设备侧归一化）
        semantic      [H, W]     uint8 语义标签（设备侧转 long）
    ...
    def __getitem__(self, i: int) -> Dict[str, torch.Tensor]:
        ...
        rgb = self.bgr_uint8(frame["rgb"][self._camera])              # [3,H,W] BGR uint8
        ...
        return {"rgb": rgb, "semantic": semantic.to(torch.uint8),
                "depth_target": depth_target,
                "depth_inrange": depth_inrange.to(torch.uint8)}
