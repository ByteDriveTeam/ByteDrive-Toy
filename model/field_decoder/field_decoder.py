"""三场解码头：BEV 特征经独立残差与专用上采样器解码为风险/可行驶/轨迹分布场。

模块: model/field_decoder/field_decoder.py
依赖: torch, config.schema.DrivingCfg, model.residual_block.ResidualBlock,
      model.bev_upsampler.BevUpsampler, model.field_decoder.checks.field_decoder_checks
读取配置:
...
说明: 复用 BEV 专用上采样器把特征逐级 2× 放大到场分辨率（Hb·2^len(up_channels)），再由三个 1×1
      卷积头各自解码为一张场的 logit：风险场（遮挡及其后为风险）、
