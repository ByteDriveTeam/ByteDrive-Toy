...
```mermaid
flowchart TB
    RGB["单目 RGB"] --> P["PerceptionModel.extract_features"]
    ...
    VEL["ego velocity"] --> VTK["速度 Token"]
    VTK --> JOINT["2× Token→BEV Cross-Attn<br/>1× Token Self-Attn"]
    SECTOR --> JOINT
...
#### 4. BEV Encoder：查询图像，而不是简单平铺特征

`32×32=1024` 个 BEV 查询 Token 通过两层 Pre-Norm 交叉注意力查询 `12×24=288` 个图像 Token。
注意力使用 PyTorch 原生 `scaled_dot_product_attention`，可在支持的设备上走 Flash/内存高效内核；
随后 8 个 ConvNeXt2D 块在 BEV 平面传播局部空间信息。
...
| `drivable` | HDMap 车道折线按 ego 位姿栅格化 | 可行驶道路区域 |
| `distribution` | 未来 GT 航点高斯软化 | 期望未来轨迹经过位置的空间分数场 |

#### 6. 轨迹与行为联合 Token

90° 前向视场被均分成 8 个扇区。每个扇区对其覆盖的 BEV cell 做均值池化，并拼接扇区中心方向
`(sin θ, cos θ)` 形成轨迹 Token；另追加 1 个可学习行为 Token。查询上下文由 1024 个 BEV Token 和
1 个归一化自车速度 Token 组成。两层交叉注意力和一层自注意力后：
...
flowchart LR
    ...
    MAP["TownXX_HD_map.npz"] --> POLY["车道折线变换到 ego"] --> RASTER["按车道宽度栅格化"] --> D["Drivable"]
    D --> EDT["距离变换"] --> OFF["Off-road distance"]
...
| `boundary` | 对全部模态/航点可微采样 HDMap 道路外距离；超出 BEV 另加坐标越界距离 |
...
| `model.driving.fields/trajectory/behavior` | 三场、轨迹和行为输出 |
...
inputs = [sample[name].unsqueeze(0).to(device) for name in
          ("rgb", "intrinsics", "extrinsics", "ego_velocity", "target_point")]
...
模型输出的三场、置信度和行为都是 logits；推理显示时分别使用 `sigmoid`，轨迹模态选择可对
`confidence` 使用 `softmax`/`argmax`。轨迹坐标已经是 ego 系米制 `(x,y)`，无需 Symlog 逆变换。
