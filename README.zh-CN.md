...
| 闭环驾驶 | CARLA 同步仿真；双帧推理；轨迹重排；世界系滚动计划；速度/曲率自适应纯追踪；行为标签停车；PID 与录像 |
...
flowchart TB
    ...
    subgraph CLOSED["CARLA 行为克隆闭环"]
        ...
        CTRL["世界系滚动计划<br/>自适应纯追踪 + 标签停车/PID"]
...
flowchart LR
    ...
    H --> R["安全与路线重排"]
    P --> R
    R --> C["世界系滚动融合<br/>自适应纯追踪 + 标签停车/PID"]
    C --> E["CARLA 下一时刻"]
    E --> S
...
在线推理可通过配置在模型置信度之外加入风险场、可行驶场和导航目标方向代价。`clone_loop` 沿每条候选轨迹采样各场，再比较终点与导航目标的
...
选中轨迹不会在每个 tick 直接替换控制目标。闭环先把它锚定到世界系：近端 0.5 秒继续执行旧计划，随后
0.5 秒线性融合新预测，远端采用新预测。纯追踪前视距离随速度增长、随路径曲率缩短，并受承诺/融合稳定时域
对应的实际路径距离限制，避免重新追逐尚未稳定的远端点。障碍停车和红灯停车两个行为标签经进入/释放双阈值
门控目标速度，PID 再把时间对齐的速度误差转成油门或制动。
...
6. Winner 转到世界系并与上一轮 active plan 滚动融合；跟踪误差超过阈值或时间轴断裂时立即重建；
7. active plan 转回当前 ego 系；控制器按速度、整条路径曲率和滚动计划稳定时域自动选择前视点，再求转角并平滑；
8. active plan 给出轨迹目标速度；障碍/红灯停车概率越过进入阈值时置零，只有低于释放阈值才恢复，PID 再计算油门/刹车；
9. 控制量施加到车辆，世界推进一个 tick；输入/下一观测、原始预测、实际参考轨迹及误差诊断一并记录，然后回到第 1 步。

第 9 步之后**没有**“把失败帧加入 DataLoader 并继续反向传播”。因此这条链路虽然是完整的控制反馈环，训练参数却冻结；
...
采集模块的完整设计和字段说明见
[data/carla_data_collector/README.zh-CN.md](data/carla_data_collector/README.zh-CN.md)。
...
- `episode_XXXX.jsonl`：时间对齐的输入/下一观测、模型决策、世界系滚动参考、动态前视/停车诊断与实际控制；
...
[`clone_loop/README.zh-CN.md`](clone_loop/README.zh-CN.md)。
...
│   ├── control/                      # 世界系滚动轨迹、自适应纯追踪、标签停车与纵向 PID
│   ├── worker/                       # Python 3.7 CARLA 同步仿真 worker
│   ├── recorder/                     # JSONL、summary 与双路 MP4
│   ├── README.md                     # 闭环英文主文档
│   └── README.zh-CN.md               # 闭环中文文档
├── Doc/
│   ├── Index.md                      # 英文文件与文档索引
│   ├── Index.zh-CN.md                # 中文文件与文档索引
│   ├── DevelopmentGuidelines.md      # 英文开发约定
│   └── 开发规范.md                   # 中文开发约定
...
- 当前闭环仅面向 CARLA 0.9.15 同步仿真，使用滚动计划、自适应纯追踪、标签停车和纵向 PID，不等同于真实车辆部署能力；
...
- 文件导航：[Doc/Index.zh-CN.md](Doc/Index.zh-CN.md)
- 开发规范：[Doc/开发规范.md](Doc/开发规范.md)
- 采集模块详解：[data/carla_data_collector/README.zh-CN.md](data/carla_data_collector/README.zh-CN.md)
- 闭环模块详解：[clone_loop/README.zh-CN.md](clone_loop/README.zh-CN.md)
