...
轨迹安全重排
世界系滚动计划 + 纯追踪/PID ─ JSON control ─► apply_control → world.tick
...
- Winner 先锚定到世界系：近端 0.5 秒保留旧计划、随后 0.5 秒融合新预测、远端采用新预测。
- 纯追踪前视距离随速度增长、随路径曲率缩短，并受承诺/融合稳定时域内的路径距离限制。
- 障碍停车/红灯停车标签经进入与释放双阈值门控目标速度；跟踪误差超过 2m 或仿真时间断裂时重建计划。
...
- `episode_XXXX.jsonl`：时间对齐的输入/下一观测、原始预测、active reference、跟踪诊断与控制；
- `episode_XXXX_driving.mp4`：左/前/右三目横向拼接驾驶实况，包含终态帧；
- `episode_XXXX_inference.mp4`：每次模型推理的三行诊断画布，包含相机/HUD、三场、
  道路线、交通控制、全部候选轨迹与实际 active reference；
- `summary.json`：成功率与每条路线的终态、进度、里程、压线等摘要。

闭环默认加载 `train/ckpt/driving/driving.pt`。检查点不存在或非骨干权重覆盖率低于配置阈值时会硬失败，
避免随机模型被误用于车辆控制。

横向控制仍使用纯追踪而非转向 PID，但输入已改为世界系持久计划转回当前 ego 系后的 reference。
`lookahead_min_m/max_m`、`lookahead_time_s` 和 `lookahead_curvature_gain` 自动调节追踪距离，滚动计划稳定时域
再提供上限；`behavior_stop_threshold` 与 `behavior_stop_release_threshold` 为配置的停车标签提供迟滞。
...
默认置信度权重为 `1.0`，其余三项当前默认均为 `0.0`，因此未通过环境配置开启时不会改变模型 Mode 排序。
置信度使用未归一化 logit，启用其他项后并非天然同尺度，应结合 JSONL 的 `mode_scores` 和推理录像调参。
